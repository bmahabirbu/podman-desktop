...
export class ImageRegistry {
   ...
   *   For multi-arch images: Docker stores only the manifest list digest, Podman stores both list and platform digests.
   *   For single-arch images: Both runtimes store only the platform digest.
  ...
  async checkImageUpdateStatus(
    ...
    // Check if the image is already up to date by comparing digests.
    //
    // Digest comparison strategy:
    // - Multi-arch images (listDigest is defined): Use the manifest list digest for comparison.
    //   Both Docker and Podman store the list digest in RepoDigests for multi-arch images.
    //   (Podman also stores the platform digest, but list digest is common to both runtimes)
    // - Single-arch images (listDigest is undefined): Use the platform digest directly.
    //   Both runtimes store only the platform digest for single-arch images.
