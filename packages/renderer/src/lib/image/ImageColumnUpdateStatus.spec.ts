...
test('Expect Updating state when updateInProgress is true', async () => {
  ...
  const text = screen.getByText('Updating');
  expect(text).toBeInTheDocument();
});
...
test('Expect clickable error to dispatch toggle-task-manager event', async () => {
  const image: ImageInfoUI = {
    ...baseImage,
    updateStatus: {
      status: 'error',
  ...
  expect(button).toHaveAttribute('title', 'Click to view error details in Task Manager');

  await fireEvent.click(button);

  expect(dispatchEventSpy).toHaveBeenCalledWith(expect.any(CustomEvent));
  const event = dispatchEventSpy.mock.calls[0]?.[0] as CustomEvent;
  expect(event.type).toBe('toggle-task-manager');
});
